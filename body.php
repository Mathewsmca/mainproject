...
<div id="bg" class="pt-2 pb-2">
    <h1 class="text-center mt-lg-5 pt-lg-5 mt-sm-0 pt-sm-0 font-weight-bold">Book a City Taxi to your destination</h1>
    ...
    <section class="container-fluid box col-lg-4 col-sm-10 col-xs-12 col-md-7 ml-lg-5 ml-md-5 pt-lg-4 mt-lg-4 pt-sm-0 mt-sm-0 mb-5 pb-3 pt-2">
      <div class="text-center">
        <div class="tup1">
          <button class="btn btn-primary green btn-sm tup font-weight-bold">BOOK TAXI</button><hr>
        </div>
          ...
          <div class="form-group  row feilds ">
            ...
            <select name="cabtype"  class="form-control-plaintext col-sm-9 arro" id="cabtype">
              ...
              <?php
                  $adm = new adminwrk();
                  $admc = new dbcon();
                  $show = $adm->cabtype($admc->conn); 
                  ?>
                  <?php foreach($show as $key=>$val){?>
                  <option value="<?php echo $val['ckeys']; ?>"><?php echo $val['types']; ?></option>
                  <?php } ?>
            </select>
        </div>
