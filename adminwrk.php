...
class adminwrk
{
    ...
    function newcab($conn, $sts)
    {
        $sql = "SELECT * FROM tbl_cab $sts";
        $result = $conn->query($sql);
        $appr=array();
        while($row = $result->fetch_assoc()){
            array_push($appr, $row);
    ...
    function cyesno($ap,$id,$conn)
    {
        if($ap==3)
        {
            $sql = "UPDATE tbl_cab SET status=3 WHERE id=$id";
        ...
        if($ap==2)
        {
            $sql = "UPDATE tbl_cab SET status=2 WHERE id=$id";
        ...
        if($ap==0)
        {
            $sql = "DELETE FROM tbl_cab WHERE id=$id";
    ...
    function alocation($route,$location,$distance,$available,$conn)
    {
        $errors=array();
        ...
        if(count($errors)==0 )
        {   
            $sql = "INSERT INTO location(`name`, `distance`, `is_available`,`route`) VALUES('".$location."', '".$distance."', '".$available."','".$route."')";
    ...
    {
        $sql = "SELECT * FROM ride WHERE status=4";
        $result = $conn->query($sql);
        $appr=array();
        while($row = $result->fetch_assoc()){
            array_push($appr, $row);
