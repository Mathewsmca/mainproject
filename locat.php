...
if (isset($_POST['submit']))
{
    ...
    $adm = new adminwrk();
    $admc = new dbcon();
    $show = $adm->alocation($route,$location,$distance,$available,$admc->conn);
  }
...
<div >
  ...
  <label class="col-sm-2" for="route" >Route</label>
    <select name="route" class="form-control">
    <?php 
           $adm = new adminwrk();
           $loc = new dbcon();
           $sloc = $adm->getrout($loc->conn);
           foreach($sloc as $key=>$val)
           {
              ?>
                <option value="<?php echo $val['id']?>"><?php echo $val['route']?></option>
              <?php
           }
      ?>
    </select>
